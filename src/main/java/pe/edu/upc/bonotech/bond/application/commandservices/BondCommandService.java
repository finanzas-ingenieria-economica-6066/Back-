...
public class BondCommandService implements IBondCommandService {
    ...
    @Override
    public Optional<Bond> updateAndRecalculateBond(Long bondId, CreateBondCommand command) {
        var optionalBond = bondRepository.findById(bondId);

        if (optionalBond.isEmpty())
            return Optional.empty();

        var bond = optionalBond.get();

        // Actualizar campos con los nuevos valores
        updateBondFromCommand(bond, command);

        // Recalcular todo
        bond.getFlows().clear();
        bond.setResultsFlow(null);
        
        generatePaymentSchedule(bond);
        calculateFinancialResults(bond);

        return Optional.of(bondRepository.save(bond));
    }

    @Override
    public Optional<Bond> partialUpdateAndRecalculateBond(Long bondId, UpdateBondResource resource) {
        return bondRepository.findById(bondId).map(bond -> {
            // Actualizar solo los campos proporcionados
            updateBondFromResource(bond, resource);
